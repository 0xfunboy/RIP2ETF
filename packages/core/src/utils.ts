...
const DEFAULT_SEGMENT = '00000000';

const SECRET_REPLACERS: Array<(value: string) => string> = [
  (value) => value.replace(/sk-[\w-]+/gi, 'sk-****'),
  (value) => value.replace(/(bearer\s+)[^\s]+/gi, '$1***'),
  (value) => value.replace(/(authorization:\s*bearer\s+)[^\s]+/gi, '$1***'),
  (value) => value.replace(/(api[_-]?key=)[^&\s]+/gi, '$1***'),
];

const sanitizeSegment = (raw?: string | UUID) => {
...
export const isDebugFlagEnabled = (flag: string): boolean => {
  ...
  if (!raw) return false;
  return TRUTHY_DEBUG_VALUES.has(raw.toLowerCase());
};

export const sanitizeLogPreview = (input: string, maxLength = 1500): string => {
  if (!input) return '';
  const trimmed = maxLength > 0 && input.length > maxLength ? input.slice(0, maxLength) : input;
  return SECRET_REPLACERS.reduce((acc, replacer) => replacer(acc), trimmed);
};

export const ensureCorrelationId = (
  state?: State,
  message?: Memory,
  preferredId?: string
): string => {
  const stateCorr = (state?.values?.corrId as string | undefined) ?? undefined;
  const contentCorr = message?.content?.corrId;
  const corrId = stateCorr || contentCorr || preferredId || buildCorrelationId(message);

  if (state) {
    state.values = state.values || {};
    (state.values as Record<string, unknown>).corrId = corrId;
  }

  if (message?.content) {
    message.content.corrId = corrId;
  }

  return corrId;
};

const buildCorrelationId = (message?: Memory): string => {
  const timestamp = Date.now();
  const roomSegment = sanitizeSegment(message?.roomId);
  const messageSegment = sanitizeSegment(message?.id);
  return `rip2etf:${timestamp}:${roomSegment}:${messageSegment}`;
};
