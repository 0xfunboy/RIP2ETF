...
async function loadSecretsNodeImpl(character: Character): Promise<boolean> {
  ...
  const MULTI_VALUE_KEYS = new Set(['TELEGRAM_ALLOWED_CHATS']);
  const normalizeListValue = (key: string, value: string): string => {
  ...
  try {
    ...
    const normalizedEnvVars = Object.fromEntries(
      Object.entries(envVars).map(([key, value]) => [key, normalizeListValue(key, value)])
    );
    ...
    character.settings = {
      ...normalizedEnvVars, // Lower priority: defaults from .env
