...
export class ElizaOS extends EventTarget {
  ...
  async addAgents(
    ...
    const promises = agents.map(async (agent) => {
      ...
      const conversationLengthSettingRaw =
        agent.settings?.conversationLength ??
        (character.settings?.CONVERSATION_LENGTH as number | string | undefined) ??
        (character.settings?.conversationLength as number | string | undefined) ??
        process.env.CONVERSATION_LENGTH;

      const conversationLength = (() => {
        if (conversationLengthSettingRaw === undefined || conversationLengthSettingRaw === null) {
          return undefined;
        }
        const parsed = Number(conversationLengthSettingRaw);
