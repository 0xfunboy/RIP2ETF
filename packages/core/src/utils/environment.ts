...
export function findEnvFile(): string | null {
  ...
  const explicitPath = process.env.ELIZA_ENV_FILE?.trim();
  if (explicitPath) {
  ...
  const candidatePaths = (baseDir: string) => [
    path.join(baseDir, '.env.local'),
    path.join(baseDir, '.env'),
    path.join(baseDir, '.eliza', '.env.local'),
    path.join(baseDir, '.eliza', '.env'),
  ];

  while (!visited.has(currentDir)) {
    visited.add(currentDir);
    for (const envPath of candidatePaths(currentDir)) {
