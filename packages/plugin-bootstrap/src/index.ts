...
export function shouldRespond(
  ...
  const allowedTelegramChats = parseIdList(runtime.getSetting('TELEGRAM_ALLOWED_CHATS'));
  if (sourceStr === 'telegram') {
    const candidateIds = new Set<string>();
    if (typeof room.serverId === 'string') {
      candidateIds.add(room.serverId);
    }
    if (typeof room.channelId === 'string') {
      candidateIds.add(room.channelId);
    }
    ...
    if (
      allowedTelegramChats.length === 0 ||
      Array.from(candidateIds).some((id) => allowedTelegramChats.includes(id))
    ) {
      return {
        shouldRespond: true,
        skipEvaluation: true,
        reason: 'telegram allowed chat',
      };
    }
  }
...
const postGeneratedHandler = async ({
  ...
  while (retries < maxRetries && (!responseContent?.thought || !responseContent?.actions)) {
    ...
    const parsedXml = parseKeyValueXml(response);
    if (parsedXml) {
      const parsedActions = Array.isArray(parsedXml?.actions)
        ? parsedXml.actions.map((action: string) => action?.trim?.() ?? '').filter(Boolean)
        : [];
      if (!parsedActions.find((action: string) => action.toUpperCase() === 'REPLY')) {
        parsedActions.push('REPLY');
      }
      responseContent = {
        thought: parsedXml.thought || '',
        actions: parsedActions.length > 0 ? parsedActions : ['REPLY'],
