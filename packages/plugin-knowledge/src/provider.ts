...
export const knowledgeProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory) => {
    ...
    const shouldFetchKnowledge = personaTriggers.some((phrase) => text.includes(phrase));

    if (!shouldFetchKnowledge) {
      return {
        data: { knowledge: '', knowledgeUsed: false },
    ...
    const firstFiveKnowledgeItems = knowledgeData?.slice(0, 3);
