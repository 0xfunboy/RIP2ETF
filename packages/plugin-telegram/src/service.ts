...
export class TelegramService extends Service {
  ...
  private async isGroupAuthorized(ctx: Context): Promise<boolean> {
    ...
    const rawValue = String(allowedChats).trim();
    if (!rawValue) {
      return true;
    }

    let allowedChatsList: string[] = [];

    if (rawValue.startsWith('[')) {
      try {
        const parsed = JSON.parse(rawValue);
        if (Array.isArray(parsed)) {
          allowedChatsList = parsed.map((value) => String(value).trim()).filter(Boolean);
        } else {
          logger.error(
            { rawValue },
            'TELEGRAM_ALLOWED_CHATS JSON value must be an array of chat identifiers.'
          );
        }
      } catch (error) {
        logger.error({ error }, 'Error parsing TELEGRAM_ALLOWED_CHATS as JSON');
      }
    }

    if (allowedChatsList.length === 0) {
      allowedChatsList = rawValue
        .split(/[,;\s]+/)
    ...
    if (allowedChatsList.length === 0) {
      logger.warn('TELEGRAM_ALLOWED_CHATS is defined but no valid chat identifiers were parsed.');
      return false;
    }
