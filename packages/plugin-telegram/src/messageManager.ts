...
import { sendPhotoWithFallback } from './lib/sendPhoto';
...
type TelegramMediaInput =
  ...
  | fs.ReadStream
  | {
      source: string | Buffer | fs.ReadStream;
      filename?: string;
    };
...
export class MessageManager {
  ...
  async sendMessageInChunks(
    ...
    if (content.attachments && content.attachments.length > 0) {
      for (const attachment of content.attachments) {
        const mediaType = this.detectMediaType(attachment.contentType);
        const mediaPayload = await this.resolveMediaInput(attachment);

        if (this.isImageAttachment(attachment)) {
          const buffer = this.mediaInputToBuffer(mediaPayload);
          if (!buffer) {
            logger.warn('Unable to resolve buffer for image attachment, falling back to generic send');
            await this.sendMedia(
              ctx,
              mediaPayload,
              mediaType,
              attachment.description || attachment.title || undefined
            );
            continue;
          }

          const caption = attachment.description || attachment.title || content.text || undefined;
          const result = await sendPhotoWithFallback(this.bot, ctx.chat.id, {
            buffer,
            ...
            logger.error(
              {
                ...
                fallbackError: (result.fallbackError as Error)?.message,
              },
              'tg:send:photo:err'
            );
            throw result.error || new Error('Failed to send photo attachment');
          }

          logger.info(
            `tg:send:photo:${
              result.fallback ? 'fallback' : 'ok'
            } corrId=${content.corrId} chat_id=${ctx.chat.id} filename=${
   ...
   * @param {TelegramMediaInput} media - Media content to send (URL, Buffer o stream locale).
  ...
  async sendMedia(
    ...
    try {
      ...
      await sendFunction(ctx.chat.id, media, { caption });

      logger.info(`${type.charAt(0).toUpperCase() + type.slice(1)} sent successfully`);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.error(
        { originalError: error },
        `Failed to send ${type}. Error: ${errorMessage}`
      );
      throw error;
    }
  }
  ...
  public async handleMessage(ctx: Context): Promise<void> {
    ...
    try {
      ...
      const serverId = chat.id.toString();
      const worldId = createUniqueUuid(this.runtime, serverId) as UUID;
      const worldName =
        'title' in chat && typeof chat.title === 'string' && chat.title.length > 0
          ? chat.title
  ...
  private detectMediaType(contentType?: string): MediaType {
    if (!contentType) return MediaType.PHOTO;

    const normalized = contentType.toLowerCase();

    if (normalized === ContentType.IMAGE) return MediaType.PHOTO;
    if (normalized === ContentType.VIDEO) return MediaType.VIDEO;
    if (normalized === ContentType.AUDIO) return MediaType.AUDIO;
    if (normalized === ContentType.DOCUMENT) return MediaType.DOCUMENT;

    if (normalized.startsWith('image/gif')) return MediaType.ANIMATION;
  ...
  private async resolveMediaInput(attachment: Media): Promise<TelegramMediaInput> {
    if (attachment.data) {
      ...
      if (typeof attachment.data === 'string') {
        if (attachment.data.startsWith('data:')) {
          const data = this.dataUrlToBuffer(attachment.data);
          return {
            source: data.buffer,
            filename: attachment.filename,
          };
        ...
        return {
          source: Buffer.from(attachment.data, 'base64'),
          filename: attachment.filename,
        };
      }
    }
    ...
    if (attachment.url?.startsWith('data:')) {
      const data = this.dataUrlToBuffer(attachment.url);
      return {
        source: data.buffer,
        filename: attachment.filename,
      };
    ...
    if (attachment.url?.startsWith('/') && fs.existsSync(attachment.url)) {
      return {
        source: fs.createReadStream(attachment.url),
        filename: attachment.filename,
      };
    }
    ...
    throw new Error('Attachment privo di dati o url');
  }

  private isImageAttachment(attachment: Media): boolean {
    if (attachment.contentType === ContentType.IMAGE) return true;
  ...
  private mediaInputToBuffer(media: TelegramMediaInput): Buffer | null {
    if (!media) return null;
    if (Buffer.isBuffer(media)) {
      return media;
    }
    if (typeof media === 'object' && 'source' in media) {
